...
def clip(x, clip_norm=1.0):
    divisor = jnp.maximum(jnp.linalg.norm(x) / clip_norm, 1.0)
    return x / divisor
...
def accumulate_grad(w, label_onehot, data, grad_accum, bias, clip_norm):
    update_fn = jax.vmap(
        lambda data, labels: clipped_log_likelihood_gradient(
...
def train_and_evaluate(config, workdir):
    ...
    train_ds, test_ds = dataset.get_datasets(
        config=config, data_config=data_config, batch_size=local_batch_size, repeat=True
    )
    ...
    test_x_np_list = utils.to_flat_np(test_xs, test_labels, data_config.num_labels)
    eval_step = jax.jit(
        functools.partial(
            utils.eval_step,
    ...
    accumulate_grad_pmapped = jax.pmap(
        functools.partial(accumulate_grad, bias=-10.0, clip_norm=data_config.clip),
        axis_name="batch",
    )
    accumulate_fc_pmapped = jax.pmap(
        functools.partial(accumulate_fc, clip_norm=data_config.clip), axis_name="batch"
    )
    ...
    create_noised_fc_partial = jax.jit(
        functools.partial(create_noised_fc, apply_noise_gramian=apply_noise_gramian)
    )
    ...
    fc_accum = np.zeros((data_config.hidden_dims, data_config.hidden_dims), np.float32)
    grad_accum = np.zeros((data_config.num_labels, data_config.hidden_dims), np.float32)
    ...
    fc_accum_noised = jax.device_put_replicated(fc_accum_noised, devices=jax.devices())
